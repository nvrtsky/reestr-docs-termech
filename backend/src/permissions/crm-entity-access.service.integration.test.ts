...
integration('Bitrix24 CRM entity access scope', () => {
  ...
  before(async () => {
    ...
    const rows = await database!.db.insert(registryDocuments).values([
      ...
      documentValue('one accessible linked deal is enough', 22, section.id, type.id),
    ...
    await database!.db.insert(registryDocumentLinks).values([
      ...
      linkValue(documentIds['one accessible linked deal is enough'], 'deal', 11),
      linkValue(documentIds['one accessible linked deal is enough'], 'deal', 12),
    ]);
  });
  ...
  it('shows unlinked documents and accepts access through any one linked deal', async () => {
    ...
    assert.deepEqual(
      ...
      ['allowed company', 'allowed deal', 'one accessible linked deal is enough', 'unlinked'],
    );
  });
