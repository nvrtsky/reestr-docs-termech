...
const messages: Record<string, string> = {
  ...
  attachment_storage_metadata_missing: 'Для файла отсутствуют данные размещения на Диске Bitrix24.',
  ...
  bitrix_file_move_invalid: 'Bitrix24 не подтвердил перенос файла в целевую папку.',
  ...
  bitrix_install_administrator_required: 'Установить или переустановить приложение может только администратор Bitrix24.',
  bitrix_install_application_invalid: 'Bitrix24 не подтвердил установленное приложение.',
  bitrix_import_access_denied: 'У вашей роли нет права импортировать счета или коммерческие предложения.',
  bitrix_import_update_access_denied: 'У вашей роли нет права обновлять импортированные счета или коммерческие предложения.',
  ...
  bitrix_user_not_found: 'Пользователь не найден или неактивен в Bitrix24.',
  bitrix_responsible_not_found: 'Ответственный не найден или неактивен в Bitrix24.',
  ...
  currency_invalid: 'Указан некорректный код валюты.',
  crm_access_unavailable: 'Не удалось проверить права Bitrix24 на связанные компании и сделки. Доступ временно закрыт.',
  crm_company_scope_mismatch: 'Все выбранные сделки должны принадлежать компании-контрагенту документа.',
  crm_entity_update_conflict: 'CRM-связь документа изменилась во время синхронизации. Обновите карточку и повторите действие.',
  counterparty_company_link_mismatch: 'Привязанная компания должна совпадать с компанией-контрагентом документа.',
  counterparty_company_link_required: 'Связь с компанией управляется через поле компании-контрагента.',
  ...
  deal_company_required: 'Каждая выбранная сделка должна быть связана с компанией в Bitrix24.',
  ...
  document_creation_changed: 'Документ изменился во время завершения загрузки. Обновите карточку и повторите действие.',
  document_content_delete_denied: 'Обязательное содержимое нельзя удалить без добавления другого файла или HTTPS-ссылки.',
  document_content_access_denied: 'Файлы и ссылки документа недоступны для вашей роли.',
  ...
  document_release_attachment_missing: 'У ранее принятого выпуска отсутствует сохранённый PDF.',
  document_release_auth_invalid: 'Некорректный сервисный токен выпуска документа.',
  document_release_auth_required: 'Требуется сервисный токен выпуска документа.',
  document_release_card_missing: 'Карточка принятого документа не найдена.',
  document_release_catalog_missing: 'В реестре не настроен тип для принятого документа.',
  document_release_checksum_mismatch: 'Контрольная сумма выпущенного PDF не совпадает.',
  document_release_idempotency_conflict: 'Эта версия документа уже передана с другим содержимым.',
  document_release_identity_conflict: 'Исходному документу соответствует несколько карточек реестра.',
  document_release_in_progress: 'Эта версия документа уже передаётся. Повторите попытку позже.',
  document_release_pdf_invalid: 'Переданный файл не является PDF.',
  document_release_pdf_size_invalid: 'Размер выпущенного PDF не соответствует запросу или превышает лимит.',
  document_release_portal_invalid: 'Адрес портала должен быть точным HTTPS origin.',
  document_release_portal_mismatch: 'Сервисный токен не разрешён для указанного портала.',
  document_release_state_conflict: 'Состояние выпуска изменилось во время сохранения PDF.',
  document_update_conflict: 'Документ уже изменён другим действием. Обновите карточку и повторите попытку.',
  document_transition_conflict: 'Статус документа уже изменён. Обновите карточку и повторите попытку.',
  document_archive_conflict: 'Состояние архива уже изменилось. Обновите карточку и повторите попытку.',
  document_restore_conflict: 'Документ уже восстановлен или изменён. Обновите карточку и повторите попытку.',
  ...
  historical_attachment_delete_denied: 'Историческую версию файла удалить нельзя.',
  ...
  required_document_fields_incomplete: 'Заполните все обязательные поля документа.',
  required_document_field_hidden: 'Обязательное поле нельзя скрыть от роли, которой разрешено создавать этот тип документа.',
  responsible_user_selection_required: 'Выберите ответственного из списка пользователей Bitrix24.',
  ...
  superseded_document_changed: 'Исходная редакция уже изменилась. Обновите карточку и повторите действие.',
